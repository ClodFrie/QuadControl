...
void* ioThread(void* vptr) {
    ...
    setParams(0.007265, 0.008265 + 0.004, 0.004500, 0.0011250, 0, 0);
    ...
    while (1) {  // do forever
        ...
        if (pthread_mutex_lock(&state_mutex) == 0) {
            ...
            // receive drone data
            if (cnt % 1 == 0 || data.battery_voltage == 0) {  // every n-th time or when data was not received properly
                ...
                case INIT:  // wait for measurements from Qualisys system
                    ...
                    if (Quadptr->I_z >= 200) {
                        ...
                        if (fabs(Quadptr->I_x - Quadptr->I_x_kal) < delta && fabs(Quadptr->I_y - Quadptr->I_y_kal) < delta) {
                            ...
                            initPID(&pidx, 3.2, 0.15, 2.4, 6.20, get_time_ms());  // pidX
                            initPID(&pidy, 2.9, 0.9, 1.6, 6.25, get_time_ms());   // pidY

                            initPID(&pidz, 14.5, 6.8, 4.0, 10, get_time_ms());  // pidZ
