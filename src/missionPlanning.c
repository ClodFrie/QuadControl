...
int calculateHover(double I_safeX0, double I_safeY0, double maxAngle_deg, struct QuadState* Quad, struct CONTROL* ctrl, PID* pidx, PID* pidy, PID* pidz) {
    ...
    if (t < t0) {  // takeoff trajectory
        continousPath(outputZ, t, 0, a_max, v_max, 0.33);
        Quad->trajectory.I_z = 270 + (int)(outputZ[2] * 1000);
...
int lemniscatePathX(double output[], double t, double t0) {
    if (t > t0) {
        double tOffset = 3 * M_PI;                                 // offset so that lemniscate starts in the center (0,0)
        output[0] = -1.0 / 36.0 * 1.0 / 2.0 * cos((t - t0 + tOffset) / 6); /*acceleration*/
        output[1] = -1.0 / 6.0 * 1.0 / 2.0 * sin((t - t0 + tOffset) / 6);  /*speed*/
        output[2] = 1.0 / 2.0 * cos((t - t0 + tOffset) / 6);           /*position*/
    }
...
int lemniscatePathY(double output[], double t, double t0) {
    if (t > t0) {
        double tOffset = 3 * M_PI;                                // offset so that lemniscate starts in the center (0,0)
        output[0] = -1.0 / 9.0 * 1.0 / 2.0 * sin((t - t0 + tOffset) / 3); /*acceleration*/
        output[1] = 1.0 / 3.0 * 1.0 / 2.0 * cos((t - t0 + tOffset) / 3);  /*speed*/
        output[2] = 1.0 / 2.0 * sin((t - t0 + tOffset) / 3);          /*position*/
    }
    output[3] = t0 + M_PI * 3 * 4; /* time */
}
